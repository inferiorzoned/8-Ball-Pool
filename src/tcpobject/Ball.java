...
public class Ball {
	...
	Ball(Group root,String name,int ballNo, double x, double y, double radius, double velocity) {
	...
	void updatePosition(double elapsed) {
		...
		//System.out.println("inside update ball position");
	}
	void ballCollisions(Ball b){
		double deltaX = b.getCenterX() - this.getCenterX() ;
		double deltaY = b.getCenterY() - this.getCenterY() ;
		if (colliding(this, b, deltaX, deltaY)) {
			String path = Test.class.getResource("/ball_colliding.mp3").toString();
	...
	public void bounce(Ball b1, Ball b2, double deltaX, double deltaY){
	...
	private boolean colliding(Ball b1, Ball b2, double deltaX, double deltaY) {
	...
	void detectWallCollisions() {
		if (getCenterX() < (getRadius()+92)){
			setCenterX(getRadius()+92);
			vx = -vx;
		} else if (getCenterX() > (761 - getRadius())) {
			setCenterX(761 - getRadius());
			vx = -vx;
		}
		if (getCenterY() > (444 - getRadius())) {//getScene().getHeight()
			setCenterY(444 - getRadius());
			vy = -vy;
		}else if(getCenterY() >50 && getCenterY() < (190 + getRadius())){
			setCenterY(190 + getRadius());
			vy = -vy;
		}
	...
	public String potCheck(){
		...
		if(dis1 <= 9 || dis2 <= 9 || dis3 <= 9 || dis4 <= 9 || dis5 <= 9 || dis6 <= 9){
			...
			Try.currentlyPottedBalls.add(this);
			Try.setPottedBalls();
			return this.getName();
		}
		return null;
